...
void test_array()
{
    ...
    redis_data d(array{simple_string("str1"), error("err1"), 57, bulk_string("blk1"),
                       array{simple_string("str2"), error("err2"), nil, bulk_string("blk2"), 68},
                       nil, simple_string("str3"), error("err3"), 79, bulk_string("blk3")});
    assert(check_io(d));
}
