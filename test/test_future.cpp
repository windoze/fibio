...
void test_future()
{
    ...
    fiber([](promise<int> p) { p.set_value(9); }, std::move(p)).detach();
...
void test_async()
{
    assert(async(f2(100), async(f1, 42).get(), async(f1, 24).get()).get()
           == f2(100)(f1(42), f1(24)));
}
...
{
    future<void> f0 = async([]() { this_fiber::sleep_for(std::chrono::seconds(1)); });
    future<int> f1 = async([]() {
        this_fiber::sleep_for(std::chrono::milliseconds(100));
        return 100;
    });
...
void test_wait_for_any2()
{
    ...
    for (size_t i = 0; i < 10; i++) {
        fv.push_back(
            async([i]() { this_fiber::sleep_for(std::chrono::milliseconds(100 * (i + 1))); }));
    }
...
{
    auto start = std::chrono::system_clock::now();
    auto f0 = async([]() { this_fiber::sleep_for(std::chrono::seconds(1)); });
    auto f1 = async([]() {
        this_fiber::sleep_for(std::chrono::milliseconds(100));
        return 100;
    });
...
void test_then3()
{
    ...
    auto f1
        = f0.then([](shared_future<int>& f) { return boost::lexical_cast<std::string>(f.get()); });
    p.set_value(100);
    assert(f1.get() == std::string("100"));
}
...
void test_then4()
{
    auto f
        = async([]() { return 100; })
              .share()
              .then([](shared_future<int>& f) { return boost::lexical_cast<std::string>(f.get()); })
              .share()
              .then([](shared_future<std::string>& f) { return boost::lexical_cast<int>(f.get()); })
              .share();
    assert(f.get() == 100);
}
