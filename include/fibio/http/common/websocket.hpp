...
struct frame_header
{
    ...
    void opcode(OPCODE v)
    {
        data_[0] &= 0xF0;
        data_[0] |= uint8_t(v) & 0x0F;
    }
    ...
    void ext_payload_len_16(uint16_t v)
    {
        data_[2] = v >> 8;
        data_[3] = v & 0xFF;
    }
    ...
    {
        uint64_t ret = uint64_t(data_[2]) << 56 | uint64_t(data_[3]) << 48
                       | uint64_t(data_[4]) << 40 | uint64_t(data_[5]) << 32
                       | uint64_t(data_[6]) << 24 | uint64_t(data_[7]) << 16
                       | uint64_t(data_[8]) << 8 | uint64_t(data_[9]);
        return ret;
    }
    ...
    mask_type masking_key() const
    {
        ...
        return mask_type{{data_[idx], data_[idx + 1], data_[idx + 2], data_[idx + 3]}};
    }
    ...
    size_t size() const { return payload_len_end_idx() + (mask() ? 4 : 0); }
...
struct connection
{
    ...
    void masking_key(const mask_type& mask)
    {
        masked_ = true;
        masking_key_ = mask;
    }
    ...
    {
        // static_assert(sizeof(typename std::iterator_traits<OutputIterator>::value_type)==1, "Byte
        // buffer required");
        frame_header hdr;
        hdr.clear();
        is_ >> hdr;
