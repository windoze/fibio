...
{
    redis_data_type operator()(const std::nullptr_t&) const { return RDT_NIL; }

    redis_data_type operator()(const int64_t&) const { return RDT_INTEGER; }

    redis_data_type operator()(const simple_string&) const { return RDT_SIMPLE_STRING; }

    redis_data_type operator()(const error&) const { return RDT_ERROR; }

    redis_data_type operator()(const bulk_string&) const { return RDT_BULK_STRING; }

    redis_data_type operator()(const array&) const { return RDT_ARRAY; }
};

struct result_checker : boost::static_visitor<>
{
    void operator()(const error& err) const { BOOST_THROW_EXCEPTION(redis_error(err)); }
...
{
    output_vistor(std::ostream& os) : os_(os) {}

    void operator()(const std::nullptr_t&) const { os_ << C_NULL_STRING << "-1\r\n"; }

    void operator()(const int64_t& v) const { os_ << C_INTEGER << v << "\r\n"; }
...
bool parse_redis_data(std::istream& is, redis_data& v)
{
    ...
    case C_ARRAY: {
        ...
        for (int64_t n = 0; n < i; n++) {
            redis_data e;
            if (!parse_redis_data(is, e)) return false;
